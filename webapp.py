...
.recommendation { border-top:1px solid var(--line); padding-top:30px; }.recommendation h2 { font-size:13px; color:var(--coral); text-transform:uppercase; letter-spacing:.1em; }.practice { font-size:34px; font-weight:800; letter-spacing:-.04em; margin:10px 0; }.detail { color:var(--muted); line-height:1.6; }
footer { border-top:1px solid var(--line); padding-top:18px; color:var(--muted); font:11px 'DM Mono', monospace; }
@media (max-width:760px) { main{padding:24px 20px 40px}.hero{grid-template-columns:1fr;gap:36px;padding:62px 0 48px}h1{font-size:54px}.panel{padding:22px}.bpm{font-size:64px} }
</style>
</head>
<body>
<main>
...
<section class="hero">
<div><div class="kicker">Hourly biometric guidance</div><h1>Breathe with your body's signal.</h1><p class="lede">Connect Garmin to turn your current heart rate into a gentle, timely pranayama practice. Your data stays on this local app.</p></div>
...
<section class="recommendation"><h2>Next practice</h2><div class="practice" id="practice">Connect Garmin to begin</div><p class="detail" id="detail">The app will suggest a calming, balancing, or energizing practice each hour based on your latest reading.</p></section>
    ...
    def do_GET(self):  # noqa: N802
        ...
        if path == "/":
            ...
            try:
                ...
                practice = get_pranayama_for_heart_rate(reading.bpm)
                label = practice.split(" (")[0]
                self._json({"bpm": reading.bpm, "timestamp": reading.timestamp, "practice": practice, "detail": DETAILS.get(label, "Keep the breath comfortable and steady.")})
